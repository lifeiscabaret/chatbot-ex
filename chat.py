...
#     st.session_state['session_id'] = str(uuid.uuid4())
#     ## 출력
#     # st.write(f"세션 ID가 생성되었습니다.: {st.session_state.session_id}")
...
# st.query_params.update({'age': 27})


## key: session_id
## value: UUID

## 더 많이 실행되는 조건을 위에 위치해야함.
## Query parameter
...
if 'session_id' in query_params:
    ...
    # print(uuid.uuid4())
    st.query_params.update({'session_id': session_id}) #여긴 왜 st를 써야하지? 위에 변수는 dict만 저장한것. streamlit에 있는 파람에 (원본에) 접근해야함.
